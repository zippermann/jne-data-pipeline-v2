# JNE Data Pipeline — Claude Context
...
scripts/
  ...
  governance/
    ...
    main.py       — Entry point: load → prepare aggregates → score
    report.py     — Generates _inline and _summary CSVs from score output
...
data/
  ...
  dq_output.csv / _inline.csv / _summary.csv  — DQ outputs (gitignored)
```

---

## Running the Governance Scripts

Always run both scripts and always output to `data/`. Replace the filename with whichever sample CSV you have locally:
...
python3 report.py \
  --input  ../../data/<your_sample>.csv \
  --scores ../../data/dq_output.csv
```

This produces three files: `dq_output.csv`, `dq_output_inline.csv`, `dq_output_summary.csv`.
